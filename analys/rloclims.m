...
%   $Revision: 1.9 $  $Date: 2000/06/14 19:59:14 $
...
if ~isempty(jinf),
   ...
   len = max(min(xl,yl));            % max common length 
   len = min([len ; abs(e-s)]);   % adjust to actual data extent 
   r = [rclip ; s + len * t];        % add last visible asymptote points 
end
