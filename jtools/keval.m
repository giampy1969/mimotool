function [Po,Pi,mv] = ...
  keval(G,K,str,w,FoW,FiW,SoW,SiW,MoW,MiW,ToW,TiW,bMo,bMi,bTo,bTi)

% [Po,Pi,mv]=keval(G,K,str,w); linear control evaluation.
% Performs analysis on the closed loop obtained by connecting the
% controller K as a negative output to input feedback on G (u=-Ky).
% Returns also :
...
% Default str is 'dp', type keval for more info.
% See also keval2 and keval3.
...
if nargin<2,error('Please read keval help');end

[ty,no,ni,ns]=minfo(G);
[tyk,nok,nik,nsk]=minfo(K);
