function [tout,yout]=ode78(F,tspan,y0,tol,trace,par)

% ODE78  Integrates a system of ordinary differential equations using
% 7th order formulas.
%
% [tout,yout]=ode78(F,tspan,y0,tol,trace);
%
% INPUT:
% F     - String containing name of user-supplied problem description.
%         Call: yprime = fun(t,y,par) where F = 'fun'.
%         t      - Time (scalar).
%         y      - Solution column-vector.
%         par    - an optional parameter vector to be passed to F.
%         yprime - Returned derivative column-vector; yprime(i) = dy(i)/dt.
% tspan = [t0 tfinal] (t0 = Initial value of t, tfinal = Final value of t).
...
% The main loop
   while (t < tfinal) & (h >= hmin)
      ...
      f(:,1) = feval(F,t,y,par);
      for j = 1: 12
         f(:,j+1) = feval(F, t+alpha(j)*h, y+h*f*beta(:,j),par);
      end
